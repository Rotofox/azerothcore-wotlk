...
Top-level map of the repo (branch `Playerbot`). **Note: there is no
...
| `modules/` | **Module system**: `CMakeLists.txt` (module discovery/build + `MOD_PLAYERBOTS` special case), `ModulesLoader.cpp.in.cmake` (generates `AddModulesScripts()`), `ModulesScriptLoader.h`, `create_module.sh`, `how_to_make_a_module.md` — plus the **13 installed `mod-*` modules** (9 git clones + 4 untracked local modules; gitignored; see [modules.md](modules.md)) and the broken `TrackingModule.h/.cpp` WIP (see [divergences-and-status.md](divergences-and-status.md)). |
